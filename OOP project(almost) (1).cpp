...
class bill:public restaurant{
	...
	public:
		...
		{
			srand(time(NULL));
			string guy[]={"Arkam","Hassan","Alex","Hina","Faisal","Kamra","Rohan","Rabia","Tariq"};
			int index=rand()%5;
...
void totalod(){
		 cout<<"LIFE SUCKS";
		cou.open("totalorders.txt",ios::in |ios::out);
		counts++;
		cou<<counts;
		cou.close();
	}
...
void control_panel(){
	...
	cout<<"5:for checking total orders in a day"<<endl<<"6: for checking total revenue";
	cout<<endl;
	int v;
	cin>>v;
	if(v==1){
	int tst;
	cout<<"Enter the total number of stock that you want to set"<<endl;
	cin>>tst;
	set_tstock(tst);
	admin::get_tstock();
}
	...
	cout<<"Enter the updated amount of employees, current employees are"<<endl;
	int en;
	cin>>en;
	set_enumb(en);
}
else if(v==4){
	cout<<"Current employeees are"<<endl;
	get_enumb();
}
else if(v==6){
	get_totalcash();
	...
	cou.open("totalorders.txt",ios::app);
	int kk;
	cou>>kk;
	cou.close();
	cout<<kk;
